...
local function read_docmap_js(path)
	local script = [[
const fs = require("fs");
const vm = require("vm");
...
process.stdout.write(JSON.stringify(sandbox.docmap));
]]
	...
	if result.code ~= 0 then
		error("Could not parse SuperCollider docmap.js:\n" .. (result.stderr or "unknown Node error"))
	end

	local ok, docmap = pcall(vim.json.decode, result.stdout)
	if not ok then
		error("Could not decode converted docmap: " .. tostring(docmap))
	end
...
local function generate(callback)
	require("scnvim").eval("SCDoc.helpTargetDir", function(dir)
		if not dir then
			error("Could not determine SCDoc.helpTargetDir")
		end

		local path = dir .. path_sep .. "docmap.js"
		local docmap = read_docmap_js(path)
